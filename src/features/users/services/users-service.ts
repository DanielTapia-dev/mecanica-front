...
export const usersService = {
  ...
  async listUsuarioRoles(usuarioId: string, options?: UsersRequestOptions): Promise<UsuarioRol[]> {
    const payload = await requestUsersApi<unknown>(
      usersApiPaths.usuarioRoles(usuarioId),
      options
    )
    return getListData<UsuarioRol>(payload, ["usuario_roles", "usuarioRoles", "data"])
  },

  async listUsuarioRolesDetalle(options?: UsersRequestOptions): Promise<UsuarioRolDetalle[]> {
    const payload = await requestUsersApi<unknown>(usersApiPaths.usuarioRolesDetalle, options)
